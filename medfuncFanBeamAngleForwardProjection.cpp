...
std::vector<std::vector<double>> medfuncFanBeamAngleForwardProjection(int N, std::vector<double>beta, double SOD, int N_d, double delta_gamma) {
    ...
    for (int i = 0; i < beta.size(); i++) {
        ...
        for (int j = floor(N_d / 2); j > -floor(N_d / 2); j--) {
            cv::Point2f boundary(0, (rotatedImage.rows - 1) / 2.0 -abs(focus1.x) * tan(j * delta_gamma));
            int delta_x = 0;
            double limity = boundary.y - delta_x * tan(j * delta_gamma);
            while (round(limity) >= 0 && round(limity) < rotatedImage.rows&& delta_x< rotatedImage.cols) {
                ...
                delta_x++;
                limity = boundary.y - delta_x * tan(j * delta_gamma);
            }
